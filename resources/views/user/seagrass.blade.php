    ...
    <div class="container mt-3">
        ...
        <form action="{{ route('user.seagrass.search') }}" method="GET" class="form-inline mb-3">
            <div class="input-group">
                <input type="text" name="query" class="form-control form-control-sm" placeholder="Search by name or location" value="{{ request()->input('query') }}" style="width: 50px;">
    ...
    @else
        @foreach ($myEntry as $d)
            ...
            <div class="container mt-3">
                <div class="row">
                    <div class="col-sm-3">
                        <img src="{{ asset('storage/' . $d->photo) }}" alt="Seagrass Photo" class="img-fluid rounded shadow-sm" id="imageToSelect-{{ $d->id }}">
                    </div>
                                    ...
                                    <div class="row mt-2">
                                        ...
                                        <div class="col-sm-9 d-flex justify-content-end align-items-center">
                                            <a href="javascript:void(0);" class="p-4 likeBtn" data-id="{{ $d->id }}">
                                                ...
                                                <span id="like-count-{{ $d->id }}">{{ $interactions->firstWhere('seaviews_id', $d->id)->total_likes ?? 0 }}</span>
                                            </a>
                                            <a href="javascript:void(0);" class="dislikeBtn" data-id="{{ $d->id }}">
                                                <i class="fas fa fa-thumbs-down fa-2x text-red-300"></i>
                                                <span id="dislike-count-{{ $d->id }}">{{ $interactions->firstWhere('seaviews_id', $d->id)->total_dislikes ?? 0 }}</span>
                                            </a>
                                            <p class="text-danger pl-4 mt-4">
                                                <span id="view-count-{{ $d->id }}">{{ $interactions->firstWhere('seaviews_id', $d->id)->total_views ?? 0 }}</span>
