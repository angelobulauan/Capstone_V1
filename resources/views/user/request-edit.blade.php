...
<body>
    ...
    @section('content')
        <form id="seagrassForm" action="{{ route('request.requests.update', ['id' => $seaview->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="container">
                    ...
                    <div class="card-header">
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" name="scientificname1" class="form-control" autocomplete="off"
                                    placeholder="Scientific Name of the Sea Grass" value="{{ $seaview->scientificname1 }}">
                            </div>
                            <div class="col-md-6">
                                <label for="scientificname" class="form-label">Scientific Name 2</label>
                                <input type="text" name="scientificname2" class="form-control" autocomplete="off"
                                    placeholder="Scientific Name of the Sea Grass" value="{{ $seaview->scientificname2 }}">
                            </div>
                            <div class="col-md-6">
                                <label for="scientificname" class="form-label">Scientific Name 3</label>
                                <input type="text" name="scientificname3" class="form-control" autocomplete="off"
                                    placeholder="Scientific Name of the Sea Grass" value="{{ $seaview->scientificname3 }}">
                            </div>
                            <div class="col-12">
                                <label for="description" class="form-label">Description</label>
                                <textarea class="form-control" name="description" rows="3" autocomplete="off" placeholder="Describe the Location">{{ $seaview->description }}</textarea>
                            </div>
                            <span></span>
                            <div class="row">
                                <div class="col">
                                    ...
                                    <input type="text" name="location" class="form-control" autocomplete="off"
                                        placeholder="Enter the Location" required value="{{ $seaview->location }}">
                                </div>
                            </div>
                            ...
                            <div class="row mb-1 mt-1">
                                ...
                                <div class="col-sm-12">
                                    <input type="file" name="photo[]" class="form-control border" multiple
                                        onchange="previewMultipleImages(event)">
                                </div>
                            </div>
                            <div class="row mt-2" id="preview-all">
                                @if (!empty($seaview->photo))
                                    @foreach (explode(',', $seaview->photo) as $photo)
                                        <img src="{{ asset('storage/' . $photo) }}" class="img-fluid rounded"
                                            style="max-width: 250px; max-height: 250px; margin: 5px;">
                                    @endforeach
                                @endif
                            </div>
                            ...
                            <script>
                                function previewMultipleImages(event) {
                                    ...
                                    var container = document.getElementById('preview-all');
                            ...
                            <script>
                                ...
                                var markerAdmin;
                                var myLatlngAdmin = {
                                    lat: {{ $seaview->latitude }},
                                    lng: {{ $seaview->longtitude }}
                                };
        ...
        <!-- Ajax Form Handling with SweetAlert2 -->
        <script>
            $(document).ready(function() {
                // Handle the form submission with AJAX
                $('#seagrassForm').submit(function(event) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            $('#seagrassForm')[0].reset(); // Reset the form
                        },
